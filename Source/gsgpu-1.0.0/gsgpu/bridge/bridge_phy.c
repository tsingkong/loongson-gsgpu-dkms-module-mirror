...
bridge_phy_connector_best_encoder(struct drm_connector *connector)
{
	...
	DRM_DEBUG_DRIVER("Finding the best bridge encoder\n");
	/* pick the first one */
	drm_connector_for_each_possible_encoder(connector, encoder) {
		return encoder;
	}

	DRM_ERROR("Failed to get encoder\n");
	return NULL;
}
...
static int bridge_phy_attach(struct drm_bridge *bridge, enum drm_bridge_attach_flags flags)
{
	struct gsgpu_bridge_phy *phy = to_bridge_phy(bridge);
